...
<template>
  <div
    ref="sectionRef"
    class="transition-colors duration-700 ease-in-out overflow-x-hidden pr-hero-gradient-bg"
  >
    <section class="relative flex flex-col items-center justify-start min-h-[100dvh]">
      <div class="relative w-full flex flex-col items-center min-h-[100dvh]">
        ...
        >
          ...
          />
          <div v-else class="w-screen h-screen pr-hero-gradient-bg" />
          <div v-if="bgImageSrc" class="absolute inset-0 pr-hero-overlay" />
        </div>
        ...
        <div class="container mx-auto flex flex-col items-center justify-start relative z-10">
          <div class="flex flex-col items-center justify-center w-full h-[100dvh] relative">
            ...
            <div
              class="absolute z-[2] top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 rounded-2xl pr-hero-media-shadow will-change-[width,height]"
              :style="{
                width: `${mediaWidth}px`,
            ...
            >
              <!-- YouTube Video -->
              <div v-if="mediaType === 'video' && isYoutube" class="relative w-full h-full pointer-events-none">
                <iframe
                  :src="youtubeEmbedUrl"
              ...
              <!-- Direct Video -->
              <div v-else-if="mediaType === 'video'" class="relative w-full h-full pointer-events-none">
                <video
                  :src="mediaSrc"
                  :poster="posterSrc"
                  autoplay
              ...
              <div v-else class="relative w-full h-full">
                ...
                />
                <div
                  class="absolute inset-0 pr-hero-image-overlay rounded-xl transition-opacity duration-200"
            ...
            >
              ...
              >
                <span v-if="highlightLine === 1" class="pr-hero-highlight">{{ line1 }}</span>
              ...
              >
                ...
                <template v-else>
                  <span v-if="highlightLine === 2" class="pr-hero-highlight">{{ line2Static }}</span>
...
.pr-hero-image-overlay {
  background: rgba(0, 0, 0, 0.5);
}

.pr-hero-media-shadow {
  box-shadow: 0px 0px 50px rgba(0, 0, 0, 0.3);
}

[data-theme="light"] .pr-hero-overlay {
  background: rgba(255, 255, 255, 0.5);
}

[data-theme="light"] .pr-hero-image-overlay {
  background: rgba(255, 255, 255, 0.3);
}

[data-theme="light"] .pr-hero-media-shadow {
...
[data-theme="light"] .pr-hero-gradient-bg {
  background:
...
.pr-hero-highlight::before {
  ...
  animation: pr-hero-highlight-reveal 0.9s cubic-bezier(0.22, 1, 0.36, 1) 0.5s forwards;
}

@keyframes pr-hero-highlight-reveal {
