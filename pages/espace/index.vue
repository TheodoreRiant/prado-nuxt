...
<template>
  <ClientOnly>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
      ...
      <div class="bg-prado-surface rounded-2xl border border-prado-border p-5 flex items-center gap-4">
        <div class="w-11 h-11 rounded-xl flex items-center justify-center bg-prado-sage/15">
          <CalendarDays :size="20" class="text-prado-sage" />
        </div>
        <div>
    ...
    <div v-if="actionCards.length > 0" class="space-y-3">
      ...
      >
        <div class="flex items-start gap-3">
          ...
          <ArrowRight :size="16" class="text-prado-sage shrink-0 mt-2.5" />
        </div>
      </NuxtLink>
    </div>
