  ...
  <div>
    ...
    <div class="relative h-[45vh] min-h-[340px] flex items-end">
      <div class="absolute inset-0">
        <ImageWithFallback src="/images/fresque.png" alt="Atelier Fresque de la Protection de l'Enfance" class="w-full h-full object-cover" />
      ...
      <div class="relative z-10 max-w-7xl mx-auto px-6 pb-12 w-full">
        <p class="text-[#C18ED8] text-sm mb-3 tracking-widest uppercase">Atelier pedagogique et collaboratif</p>
        <h1 class="text-3xl md:text-4xl lg:text-5xl text-prado-text mb-3" :style="{ fontFamily: 'Poppins', lineHeight: 1.1 }">
          La Fresque de la Protection de l'Enfance
        </h1>
        <p class="text-prado-text-muted text-lg max-w-2xl">
          Decouvrir les acteurs, les parcours et les enjeux de la protection de l'enfance
        </p>
      </div>
    </div>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-5 gap-12">
        <div class="lg:col-span-3 space-y-6">
          <h2 class="text-2xl md:text-3xl text-prado-text" :style="{ fontFamily: 'Poppins' }">
            Une experience immersive et collaborative
          </h2>
          <p class="text-prado-text-muted leading-relaxed">
            Inspiree de la Fresque du climat, la Fresque de la Protection de l'Enfance met en scene les parcours de trois jeunes qui donnent a voir la complexite du systeme de protection de l'enfance et la diversite des dispositifs et metiers existants.
          </p>
          <p class="text-prado-text-muted leading-relaxed">
            Combinant jeu, echanges et reflexion commune, elle permet de mieux comprendre les parcours des enfants et la complexite du systeme qui les entoure. Pas a pas, la reconstitution esquisse une vision globale du systeme, que les participants s'approprient en relevant les liens entre les acteurs et les institutions.
          </p>
          <p class="text-prado-text-muted text-sm italic border-l-2 border-[#C18ED8] pl-4">
            "C'est un outil pedagogique interessant pour eclairer les parcours de jeunes et rendre lisible la complexite du systeme de protection. Les scenarios sont bien construits et pertinents."
            <span class="block mt-1 not-italic text-prado-text-faint">-- Remi P., Chef de service en prevention specialisee</span>
          </p>

          <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 pt-4">
            <div v-for="item in objectives" :key="item.label" class="bg-prado-surface rounded-2xl p-5 border border-prado-border">
              <component :is="item.icon" :size="22" class="text-[#C18ED8] mb-3" />
        ...
        <div class="lg:col-span-2 space-y-4">
          ...
          <div v-for="p in parcours" :key="p.nom" class="bg-prado-surface rounded-2xl p-5 border border-prado-border">
            <div class="flex items-center gap-3 mb-2">
              <span class="w-2.5 h-2.5 rounded-full" :style="{ backgroundColor: p.color }" />
              <span class="text-prado-text font-medium text-sm">{{ p.nom }}</span>
            </div>
            <p class="text-prado-text-muted text-sm pl-5.5">{{ p.desc }}</p>
          </div>

          <div class="bg-[#C18ED8]/10 rounded-2xl p-5 border border-prado-border mt-4">
    ...
    <div class="max-w-7xl mx-auto px-6 py-16">
      ...
      <h2 class="text-2xl text-prado-text mb-6 flex items-center gap-2.5" :style="{ fontFamily: 'Poppins' }">
        <Calendar :size="20" class="text-[#C18ED8]" /> Prochaines sessions
      </h2>
      <div class="space-y-3 mb-16">
        <div
          v-for="s in sessions"
          :key="s.date + s.version"
        ...
        >
          <div>
            <div class="text-prado-text">
              {{ s.date }} -- <span class="text-[#C18ED8]">{{ s.version }}</span>
            </div>
          ...
          <div class="flex items-center gap-4">
            ...
            <NuxtLink to="/contact" class="px-5 py-2 rounded-full bg-[#C18ED8] text-white text-sm hover:bg-[#a870c0] transition-colors">
              S'inscrire
            </NuxtLink>
          </div>
        </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-3xl mx-auto">
        <div class="bg-[#C18ED8]/10 rounded-2xl p-6 border border-prado-border">
          ...
          <div class="space-y-2">
            <div class="flex items-center gap-2 text-sm text-[#C18ED8]">
        ...
        <div class="bg-prado-surface rounded-2xl p-6 border border-prado-border flex flex-col justify-between">
          ...
          <NuxtLink to="/contact" class="inline-flex items-center justify-center px-6 py-3 rounded-full bg-[#C18ED8] text-white text-sm hover:bg-[#a870c0] transition-colors">
            Nous contacter
          </NuxtLink>
        </div>
      </div>
